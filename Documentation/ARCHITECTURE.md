...
└── segmentation.cpp             # pybind11 module, built as coredb._native by setup.py
```
...
`coredb/_native` is a compiled [pybind11](https://pybind11.readthedocs.io/) extension (`native/segmentation.cpp`, built via `setup.py`'s `ext_modules` alongside `pyproject.toml`'s setuptools metadata) providing a literal C++ port of `coredb/signal.py`'s binary-segmentation index search (`detect_changepoints_indices`) — same algorithm, same accumulation order, not a rewrite (no prefix-sum `O(1)` segment cost, which would be a legitimate but *separate* future optimization — bundling an algorithmic change with the native-vs-Python question would make the benchmark's before/after unattributable to one variable). The GIL is released for the duration of the native call (`py::call_guard<py::gil_scoped_release>()`) since it touches no Python objects once the input is copied into a `std::vector<double>`.

**Optional, not a hard dependency.** `coredb/signal.py` tries `from . import _native` once at import time; if unavailable (no C++ toolchain at install time), `detect_changepoints()` transparently falls back to the pure-Python path with identical results — `pip install -e .` still succeeds without a compiler, `coredb._native` just doesn't exist and everything still works, only slower. `detect_changepoints(..., use_native=True|False)` forces one path explicitly (used by `tests/test_native_segmentation.py`'s equivalence tests and `benchmarks/`' `segmentation.python`/`segmentation.native`); `GraphSignal.changepoints()`/`Database.changepoints()` have no such parameter — they always use whichever path auto-detected, keeping the public API surface unchanged.

Measured on `benchmarks/run_all.py`'s default sizes (n=365, a signal with real regime shifts, isolated from `TRACK`'s own cost): segmentation search 25.9ms (Python) → 0.40ms (native), ≈65×. On the full `changepoints.degree` benchmark (`TRACK` + segmentation together, H=150, D=365): 2.35s → 15ms end to end, combining M9's interval-sweep fix and this pilot.

This is the first native code in the project, chosen deliberately as the lowest-risk pilot (pure computation, no on-disk format touched) to prove the pybind11/MSVC toolchain works before attempting anything that touches storage. See `ROADMAP.md`'s Milestone 10 for what's next and what's deliberately still deferred (betweenness/PageRank, the ingest/write path, `VersionCore`, integer-interned ids).
