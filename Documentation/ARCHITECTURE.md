...
### Integer-interned entity/predicate ids: investigated, not built (M10 Part 5)

The original proposal's last piece — replace `spo_idx`/`ops_idx`/`relationship_lookup`/`open_by_sp_idx`'s raw UTF-8 string keys with interned integer ids, on the theory that fixed-width integer keys compare faster and pack denser into LMDB's B+tree than variable-length strings. Rather than build this speculatively, `benchmarks/bench_at_scale.py` (`python -m benchmarks.bench_at_scale`, not part of the default suite — too slow for a normal dev-loop run) tests the hypothesis directly: 50,000 facts, entities named with long, realistic 35-character strings (not the default suite's short synthetic `S0`/`O0`), a small 5-predicate vocabulary reused across every record — deliberately the scenario where string-key overhead should show up most, if it shows up at all.

It doesn't. Ingest holds ≈18K ops/sec batched (no degradation vs. the default suite's short-name numbers at smaller N), and 2000 hot-path `as_of()` lookups against a real subject+predicate match run at ≈76K ops/sec (≈13µs each) — no measurable bottleneck attributable to key size or string comparison cost, even under these adversarial-for-strings conditions. LMDB's B+tree is simply fast enough at this scale that it doesn't show up.

**Conclusion: deferred with evidence, not skipped.** Per the benchmark-gates-every-optimization principle this whole milestone has followed, building interning now would be optimizing without a measured problem — the opposite of the discipline that produced M9's TRACK/SERIES fix and M10's earlier parts. `benchmarks/bench_at_scale.py` stays in the repo as a reusable, reproducible check: if entity/relationship counts grow into the millions, or the working set exceeds available RAM (page-fault costs a 50K-record in-memory-friendly test can't reveal), re-run it — a *positive* result there would be the actual gate for building this.
