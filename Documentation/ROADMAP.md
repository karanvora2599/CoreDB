...
- **Remaining parts** (see "Explicitly deferred" below): the storage/ingest-path work from the original proposal (a compact binary encoding for data crossing into native code, integer-interned entity/predicate ids, covering indexes, a batch write API) — deliberately sequenced last since it touches storage format/schema and needs more care than the pure-computation parts above.
...
- **`VersionCore`-style compact binary encoding, integer-interned entity/predicate ids, covering indexes, a batch write API** — the original proposal's storage/ingest-path half, M10's remaining parts. Deliberately sequenced after the pure-computation native work (M10 Parts 1-2) since these touch on-disk format/schema and need a real migration story (`Database.dump()`/`coredb.restore()` already exist as the schema-change path — see "Storage management" in `ARCHITECTURE.md`) — not something to rush. M9's benchmark suite is explicitly the gate for all of this: no native optimization should land without `benchmarks/` numbers proving it actually helps.
