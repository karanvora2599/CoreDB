...
- **Remaining parts** (see "Explicitly deferred" below for the parts not yet started): native versions of the still-unoptimized global centrality computations (`betweenness_all`/`pagerank_all`), and the storage/ingest-path work from the original proposal (a compact binary encoding for data crossing into native code, integer-interned entity/predicate ids, covering indexes, a batch write API) — deliberately sequenced after the pure-computation work since they touch storage format/schema and need more care.
...
- **Native `betweenness_all`/`pagerank_all`** — M10 Part 1 covered pure-computation segmentation only; these are the next pure-computation (no storage-format change) native targets, still gated on `benchmarks/`' `track_betweenness` numbers actually improving.
- **`VersionCore`-style compact binary encoding, integer-interned entity/predicate ids, covering indexes, a batch write API** — the original proposal's storage/ingest-path half. Deliberately sequenced after the pure-computation native work (M10 Part 1, and the centrality bullet above) since these touch on-disk format/schema and need a real migration story (`Database.dump()`/`coredb.restore()` already exist as the schema-change path — see "Storage management" in `ARCHITECTURE.md`) — not something to rush. M9's benchmark suite is explicitly the gate for all of this: no native optimization should land without `benchmarks/` numbers proving it actually helps.
- **Incremental/native `TRACK CLOSENESS`/`BETWEENNESS`/`PAGERANK`** — M9's interval sweep only applies to degree-family metrics and `SERIES`; global per-date graph algorithms have no simple event-sweep equivalent (see `ARCHITECTURE.md`'s Performance section) and would need real incremental-graph-algorithm work, not a straightforward refactor. (A *native, non-incremental* betweenness/PageRank is still worth doing per the bullet above — this bullet is specifically about the harder incremental-algorithm version.)
