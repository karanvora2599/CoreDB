...
## Milestone 10 — native acceleration (in progress)
...
- **Remaining part**: integer-interned entity/predicate ids and covering indexes — the last piece of the original proposal's storage-format half, deliberately sequenced last (the most invasive change: touches every index, not just one table).
...
- **Integer-interned entity/predicate ids, covering indexes** — the original proposal's last remaining piece, M10's final part. Deliberately sequenced after everything else in M10 since it's the most invasive on-disk format change (touches `spo_idx`/`ops_idx`/`relationship_lookup`/`open_by_sp_idx` — every index, not just the `versions` table `VersionCore` (Part 4) touched) and needs a real migration story (`Database.dump()`/`coredb.restore()` already exist as the schema-change path — see "Storage management" in `ARCHITECTURE.md`) — not something to rush. M9's benchmark suite is explicitly the gate for all of this: no native optimization should land without `benchmarks/` numbers proving it actually helps.
