...
- **Remaining parts** (see "Explicitly deferred" below): `VersionCore`-style compact binary encoding, integer-interned entity/predicate ids, and covering indexes — the parts of the original proposal that touch on-disk format/schema, deliberately sequenced last.
...
- **`VersionCore`-style compact binary encoding, integer-interned entity/predicate ids, covering indexes** — the original proposal's remaining storage-format half, M10's last parts. Deliberately sequenced after everything else in M10 since these touch on-disk format/schema and need a real migration story (`Database.dump()`/`coredb.restore()` already exist as the schema-change path — see "Storage management" in `ARCHITECTURE.md`) — not something to rush. M9's benchmark suite is explicitly the gate for all of this: no native optimization should land without `benchmarks/` numbers proving it actually helps.
