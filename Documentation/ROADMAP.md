...
## Milestone 10 — native acceleration and storage optimization — done

A multi-part native (C++/pybind11) effort, executed as a sequence of checkpoints rather than one large change — each part lands, is tested and benchmarked, and is committed on its own before the next begins. This picks up the C++ proposal an external technical review raised (see M9's context); M9 already closed out that review's original top-priority target (`TRACK`/`SERIES`) in pure Python, so this milestone covers what's left, reordered by risk (lowest-risk/no-format-change work first):

- **Part 1 — done: first native pilot, `CHANGEPOINTS` segmentation.** `native/segmentation.cpp`, a literal pybind11 port of `coredb/signal.py`'s binary-segmentation index search (not an algorithmic rewrite — see `ARCHITECTURE.md`'s Performance section for why that's deliberate). Optional: falls back to pure Python transparently if no C++ toolchain is present at install time. Proved the toolchain (MSVC via VS 2022 Build Tools, auto-detected by setuptools with no pre-activated dev shell needed) and the pattern (native module behind a try/except `ImportError`, `use_native` override for tests/benchmarks) end to end. Measured ≈65× on the isolated segmentation search; ≈150× on `CHANGEPOINTS` end to end on top of M9's own fix.
- **Part 2 — done: native centrality, `betweenness_all`/`pagerank_all`.** `native/centrality.cpp` (Brandes' algorithm, power-iteration PageRank), same literal-port discipline as Part 1. `coredb/graph_algorithms.py` is new: a pure computation layer (adjacency lists of local integer indices, no LMDB/string ids) that `engine.py` builds data for and delegates to — the LMDB reads stay in `engine.py` (the only place with a transaction), keeping this pure-computation, no-storage-format-change, same as Part 1. Measured ≈5× (betweenness) and ≈12× (PageRank) on the isolated computation at n=150 nodes; `TRACK BETWEENNESS`/`PAGERANK`'s own per-resolution-step *count* is unchanged (still no event-sweep for a global algorithm), but each step got cheaper.
- **Part 3 — done: `Database.write_batch()`, the batch write API.** Not native code — a pure-Python fix for the original proposal's "`restore_batch`"/one-transaction-per-record complaint, landed as its own checkpoint since it's additive (new API, no format change) and directly measurable. `assert_fact`/`retract_fact`/`sync_snapshot` now share their transaction with an active `write_batch()` block instead of always opening their own; `coredb.restore()` uses it internally (chunked at 5000 records/transaction, not unbounded — see `ARCHITECTURE.md`'s Performance section for why). Measured ≈17× on raw ingest (2000 facts, one `write_batch()` vs. one transaction per call), ≈11× on `coredb.restore()` end to end.
- **Part 4 — done: `VersionCore`, a compact binary encoding for the `versions` table.** `coredb/storage/version_codec.py` — the first (and, deliberately, only-for-now) on-disk format change in M10: `SCHEMA_VERSION` bumped 1→2, so an old database fails loudly with `SchemaVersionError` on open rather than being misread (`dump()`/`restore()` is the migration path, per "Storage management" in `ARCHITECTURE.md`). Measured ≈1.4× on isolated encode/decode — a modest, honestly-reported number: unlike Parts 1-2 this doesn't cross into native code, both JSON and `struct` are already C-implemented in CPython, so the win is narrower than the native ports above it.
- **Part 5 — done: integer-interned entity/predicate ids and covering indexes, investigated and deferred with evidence.** The last piece of the original proposal, and the most invasive one (touches every index, not just one table) — rather than build it speculatively, `benchmarks/bench_at_scale.py` (new: a deep, opt-in benchmark not in the default suite) tested the hypothesis directly under conditions chosen to favor it (50,000 facts, long 35-character entity names, a small 5-predicate vocabulary reused everywhere). No measurable bottleneck showed up: ingest held ≈18K ops/sec, hot-path `as_of()` lookups ran at ≈76K ops/sec. Conclusion: not built, because the evidence doesn't justify it yet — the same "benchmark gates the optimization" discipline that produced M9's TRACK/SERIES fix, applied here to *not* build something. `bench_at_scale.py` stays as a reusable check to re-run if the graph grows enough (millions of entities, working set exceeding RAM) that the answer might change.

This closes the M10 arc as scoped: every part of the original technical-review proposal was either built (Parts 1-4) or investigated and deferred with concrete evidence (Part 5) — nothing left unaddressed by default, nothing built without a number behind it.
...
- **Integer-interned entity/predicate ids, covering indexes** — investigated in M10 Part 5 (`benchmarks/bench_at_scale.py`) and deferred with evidence, not left unaddressed: no measurable bottleneck showed up even under conditions chosen to favor the change (long entity names, a small reused predicate vocabulary, 50,000 facts). Worth reopening only if `bench_at_scale.py` (re-run at a larger scale, or against a real workload) shows otherwise.
- **Incremental `TRACK CLOSENESS`/`BETWEENNESS`/`PAGERANK`** — M9's interval sweep only applies to degree-family metrics and `SERIES`; global per-date graph algorithms have no simple event-sweep equivalent (see `ARCHITECTURE.md`'s Performance section) and would need real incremental-graph-algorithm work, not a straightforward refactor. M10 Part 2 made each per-date computation itself cheaper (native Brandes/PageRank) but didn't reduce how many times `TRACK` recomputes it — that harder, genuinely incremental version stays deferred.
