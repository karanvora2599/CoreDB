...
- **Milestone 10** (in progress): native (C++/[pybind11](https://pybind11.readthedocs.io/)) acceleration — [`native/`](native/), literal ports of `CHANGEPOINTS`' binary-segmentation search (~65× faster in isolation) and `betweenness_all`/`pagerank_all`'s centrality computations (~5–12× in isolation). Optional: `coredb` installs and runs fully correctly without a C++ toolchain, falling back to pure Python transparently. See `Documentation/ROADMAP.md` for what's next (the storage/ingest-path work: `VersionCore`, integer-interned ids, batch writes).
