...
- **Milestone 10** (in progress): native/storage optimization work, gated by `benchmarks/`'s numbers at every step — native (C++/[pybind11](https://pybind11.readthedocs.io/)) acceleration ([`native/`](native/): `CHANGEPOINTS`' binary segmentation ~65× faster in isolation, `betweenness_all`/`pagerank_all` ~5–12×; optional, `coredb` installs and runs correctly without a C++ toolchain, falling back to pure Python), `Database.write_batch()` (~17× on bulk ingest, ~11× on `coredb.restore()`), and `VersionCore` — a compact binary encoding for the `versions` table (schema-versioned; `SchemaVersionError` + `dump()`/`restore()` is the migration path). See `Documentation/ROADMAP.md` for what's left (integer-interned entity/predicate ids, covering indexes).
