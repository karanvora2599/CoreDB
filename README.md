...
- **Milestone 10** (in progress): first native (C++/[pybind11](https://pybind11.readthedocs.io/)) acceleration pilot — [`native/segmentation.cpp`](native/segmentation.cpp), a literal port of `CHANGEPOINTS`' binary-segmentation search (measured ~65× faster in isolation). Optional: `coredb` installs and runs fully correctly without a C++ toolchain, falling back to pure Python transparently. See `Documentation/ROADMAP.md` for what's next.
