...
- **Milestone 10** (done): native/storage optimization work, gated by `benchmarks/`'s numbers at every step — native (C++/[pybind11](https://pybind11.readthedocs.io/)) acceleration ([`native/`](native/): `CHANGEPOINTS`' binary segmentation ~65× faster in isolation, `betweenness_all`/`pagerank_all` ~5–12×; optional, `coredb` installs and runs correctly without a C++ toolchain, falling back to pure Python), `Database.write_batch()` (~17× on bulk ingest, ~11× on `coredb.restore()`), `VersionCore` (a compact binary encoding for the `versions` table; schema-versioned, `SchemaVersionError` + `dump()`/`restore()` is the migration path), and integer-interned entity/predicate ids — investigated (`benchmarks/bench_at_scale.py`) and deferred with evidence: no measurable bottleneck at 50,000 facts even under string-unfavorable conditions.
- See [`Documentation/ROADMAP.md`](Documentation/ROADMAP.md) for what's deliberately deferred beyond that scope (general multi-hop pattern matching, streaming, a server mode, a query optimizer).
